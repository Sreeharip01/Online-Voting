...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
     ...
     $connection= mysqli_connect($Server,$username,$psrd,$dbname);
         ...
         $imagename    = $_FILES['Cpicture']['tmp_name'];  
         $imagetype    = $_FILES['Cpicture']['type'];
         $imagetemploc = $_FILES['Cpicture']['tmp_name'];
         $imagesize    = $_FILES['Cpicture']['size'];

         $image = base64_encode(file_get_contents($imagetemploc));

         $query="insert into Candidate(CandidateName,Email,Mobile,Address,Description,Photo) values('$name','$email','$phone','$address','$description','$image')";
